...
pub struct KeyValueFile<K, V> {
    ...
    map: AsyncOnceLockMap<K, anyhow::Result<V>>,
...
impl<K: Serialize + DeserializeOwned + Eq + Hash + Clone + Send + Sync + 'static,
    V: Serialize + DeserializeOwned + Clone + Send + Sync + 'static> KeyValueFile<K, V> {
    pub async fn new(path: &Path) -> io::Result<(Self, KeyValueFileCleanup)> {
        ...
        for line in history.lines() {
            ...
            map[&entry.key].get_or_init(async move { Ok(value) }).await;
        }
        ...
        Ok((KeyValueFile { map, sender: tx }, KeyValueFileCleanup(Some(writer))))
    }
    pub async fn get_or_init<'a>(&'a self, key: K, value: impl 'static + Send + Future<Output=anyhow::Result<V>>) -> anyhow::Result<&'a V> {
        let sender: UnboundedSender<_> = self.sender.clone();
        let cell: &'a AsyncOnceLock<anyhow::Result<V>> = self.map.get_or_insert(Cow::Owned(key.clone()));
        let fut = async move {
            let value = value.await;
            if let Ok(value) = &value {
                sender.send(KeyValueEntry { key, value: value.clone() }).ok().unwrap();
            }
            value
        };
        let output: &'a anyhow::Result<V> = cell.get_or_init(fut).await;
...
async fn test_key_value_file() -> io::Result<()> {
    ...
    assert_eq!(2, *kvf.get_or_init(1, async { anyhow::Result::Ok(2usize) }).await.unwrap());
    assert_eq!(4, *kvf.get_or_init(3, async { anyhow::Result::Ok(4usize) }).await.unwrap());
    mem::drop(kvf);
    cleanup.cleanup().await?;
    (kvf, cleanup) = KeyValueFile::new(&path).await?;
    assert_eq!(2, *kvf.get_or_init(1, async { anyhow::Result::Ok(todo!()) }).await.unwrap());
    assert_eq!(4, *kvf.get_or_init(3, async { anyhow::Result::Ok(todo!()) }).await.unwrap());
    mem::drop(kvf);
    cleanup.cleanup().await?;
