...
use safe_once_async::sync::{AsyncLazyLock, AsyncLazyStatic};
use tokio::io;
...
pub struct LazyMmap<T> (AsyncLazyStatic<io::Result<Box<[T], MmapAllocator>>>);
...
impl<T: AnyRepr> LazyMmap<T> {
    ...
    pub fn get(&'static self) -> impl Send + Future<Output=io::Result<&'static [T]>> {
        async move {
            Ok(&**(self.0.get().await.clone_error()?))
        }
    }
}
